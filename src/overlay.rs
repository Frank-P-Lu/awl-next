...
/// Build the SUMMONED overlay for a FLAT picker kind (Goto / Theme / Command)
/// from the caller-gathered [`BuildCtx`]. Returns `None` for the navigable
/// explorers (Browse / MoveDest / Project) — those need a directory LEVEL, built
/// by [`browse_level`]. Shared by the live App (`app.rs`) and the headless replay
/// (`main.rs`) so both summon byte-identical overlays.
