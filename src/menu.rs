...
/// items are predefined) and the "Quit Awl" + its separator from the App menu
/// (Quit is `native_only`), while leaving "About Awl" alone in a now-separator-less
/// App menu.
...
mod tests {
    ...
    /// The App menu on web drops "Quit Awl" (native_only) AND its now-dangling leading
    /// separator, keeping "About Awl" alone.
    #[test]
    fn web_roster_app_menu_keeps_about_drops_quit_and_its_separator() {
        let menus = roster_for(commands::Platform::Web);
        let app = menus.iter().find(|m| m.title == "awl").unwrap();
        assert_eq!(app.items, vec![RosterItem::Routed { id: "awl.about", label: "About Awl", icon: false }]);
    }
