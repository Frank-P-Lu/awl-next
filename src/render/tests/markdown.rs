...
/// NEIGHBORHOOD AUDIT (2026-07-22, item 19, Trigger-3 sweep over the item-4
/// markdown-render classes): a task list's `MdKind::Task`/`MdKind::TaskDone`
/// spans were, like the pre-round strikethrough span, only ever proven via
/// `markdown::spans` unit tests (`open_task_marks_box_not_text`,
/// `checked_task_dims_box_and_text`) — never through a REAL GPU frame. That is
/// exactly the mechanism-vs-outcome gap item 4(a) closed for strikethrough
/// (the span existed; nobody had proven the pixels moved). This is that same
/// APPEARANCE ORACLE for the task-list neighborhood: an OPEN checkbox row
/// (`md_attrs`'s no-op transform, full content ink) and a CHECKED row (dims the
/// checkbox glyph AND the body text to `muted`, `md_attrs`'s `Task(true)`/
/// `TaskDone` arm) must ACTUALLY PAINT DIFFERENT PIXELS on the same row —
/// never just differ on paper while a transparent/identical-ink quad reaches
/// the framebuffer (the Wagtail invisible-picker-row shape).
#[test]
fn task_list_open_and_checked_rows_paint_distinguishable_real_pixels() {
    let _g = crate::testlock::serial();
    let Some((device, queue, mut p)) = headless_dqp(1200.0, 800.0) else {
        eprintln!(
            "skipping task_list_open_and_checked_rows_paint_distinguishable_real_pixels: no wgpu adapter"
        );
        return;
    };
    let w = 1200u32;
    let h = 800u32;
    // Byte-identical bodies ("buy milk"); only the checkbox state differs
    // ('[ ]' vs '[x]') — caret parked far below so nothing else moves.
    let open = "- [ ] buy milk\n\nprose\n";
    let checked = "- [x] buy milk\n\nprose\n";

    // "- [ ] buy milk" / "- [x] buy milk": byte-identical bodies, differing
    // only in the checkbox state — caret parked far below so nothing else
    // moves. (Proportional faces give ' ' and 'x' different advances, so the
    // body text is NOT column-aligned between the two rows — the region below
    // covers the WHOLE row, exactly the strike/link/fence tests' shape,
    // rather than trying to isolate a single sub-mechanism's pixels.)
    let mut a_view = view(open, 2, 0);
    a_view.is_markdown = true;
    p.set_view(&a_view);
    // Sanity: the OPEN document really does carry the span `md_attrs`
    // transforms (a state-level guard alongside the pixel one — the sidecar
    // oracle, never a SUBSTITUTE for it).
    let spans_a = p.md_report();
    assert!(spans_a.iter().any(|(_, _, t)| t == "task_open"), "{spans_a:?}");
    p.prepare(&device, &queue, w, h).unwrap();
    let a = pixeldiff::render_frame(&mut p, &device, &queue, w, h);

    let mut b_view = view(checked, 2, 0);
    b_view.is_markdown = true;
    p.set_view(&b_view);
    let spans_b = p.md_report();
    assert!(spans_b.iter().any(|(_, _, t)| t == "task_checked"), "{spans_b:?}");
    assert!(spans_b.iter().any(|(_, _, t)| t == "task_done"), "{spans_b:?}");
    p.prepare(&device, &queue, w, h).unwrap();
    let b = pixeldiff::render_frame(&mut p, &device, &queue, w, h);
    ...
    pixeldiff::assert_perceptibly_different(
        ...
        region,
        DistinguishFloor::DEFAULT,
        "a checked task row (dimmed box + dimmed body) must paint different \
         pixels than the byte-identical-body open row",
    );
}
