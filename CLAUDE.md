...
## Context-aware mouse cursor shapes (`cursor_shape.rs` + `app/input.rs`) — winit draws it, we just decide
...
## i18n — multilingual docs (`frontmatter.rs` + `script.rs` + `theme.rs` + `render/spans.rs`)
...
- **FONT-ID RESOLVER (`theme.rs` + `render/text.rs`):** `theme::FontId` {`Latin`, `Ja`, `ZhHans`, `ZhHant`, `Ko`} generalizes the old ja-only `Theme::cjk` + `resolve_cjk`. `Theme::candidates(id)` returns a prioritized family-name ladder per script — DATA, never a code path (`Latin` = the world's own `Theme::font`, a single-element always-registered floor; `Ja` = unchanged `Theme::cjk`/`CJK_MINCHO`/`CJK_GOTHIC`). `TextPipeline::resolve_font_id(id)` is `resolve_cjk`'s exact algorithm generalized: walk the ladder, return the first family registered in the font DB + its concrete weight nearest 400 (the same Hiragino/PingFang weight-trap correction). `resolve_cjk()` now delegates to `resolve_font_id(FontId::Ja)` — byte-identical ja behavior. **NEVER-TOFU LAW**, tested in two halves: `theme::tests::every_font_id_has_a_nonempty_candidate_ladder_on_every_world` (structural — no world may ship an empty ladder for any script, environment-independent) + `render::tests::latin_and_ja_always_resolve_to_an_embedded_face` (font-DB — Latin/Ja's guaranteed floor is real on every world via the real font system). `ZhHans`/`Ko` originally shipped v1 with no bundled asset — **see "THE CHINESE ROUND" below**, which bundled both; `ZhHant` is STILL system-only (`CJK_ZH_HANT` = PingFang TC → Noto Sans CJK TC) — Big5 coverage (~13k chars) remains banked, not attempted.
...
### THE CHINESE ROUND — bundled zh-Hans + ko floors (`assets/fonts/` + `theme.rs` + `render.rs`)
...
- **Tests:** the arm decision at the apply seam (`actions::tests::copy_with_selection_arms_the_copy_pulse` / `copy_without_selection_does_not_pulse` / `cut_does_not_arm_the_copy_pulse`); the caret kick's shape (`caret::tests::copy_pulse_is_the_gentlest_pure_squash_no_velocity_kick`, `_is_deliberately_not_velocity_damped`); the pure decay math (`render::tests::copy_pulse_ease_is_a_clamped_smoothstep`, `copy_pulse_settles_at_construction_then_kicks_and_decays_back`) and the pure color blend (`selection::tests::lerp4_interpolates_linearly_between_endpoints`); both `main/run.rs` and `app/apply.rs`'s exhaustive `Effect` matches were extended (a missing arm fails to compile — the completeness-sweep pattern this codebase already leans on).
...
- **Adding a `ViewState` field:** update EVERY `ViewState {` initializer — the FIVE production sites are `src/app/viewstate.rs` (the live App's `sync_view`), `src/bench.rs` (`view_for`), `src/capture/modes.rs`, `src/render/framebench.rs` (`live_view`), and `src/render/perfbench.rs` (`bench_view`); the `src/render/tests.rs` `view()` helper is a sixth (test-only) — or the build breaks only at merge time (git auto-merges cleanly and then fails to compile).
...
- **Flake (FIXED twice — suite is parallel-safe; page-global WRITERS now self-serialize):** two rounds of the same disease, one lock as the cure. (1) `render::tests::theme_font_switch_reshapes_document` used to fail under PARALLEL `cargo test`: it read geometry folding the process-global PAGE state (`column_width()` → `page_on()`/`measure()`, geometry.rs) holding only `theme::TEST_LOCK` — it and every render test reading page-folding geometry now hold both. (2) The prose/code page-width split made `replay_keys`' Goto arm, `App::sync_page_measure`, and `apply_sticky_globals` page-global writers, so the pre-existing multi-buffer Goto replay tests (which correctly never held the page lock — they weren't writers when written) stomped `run::tests::visual_*` mid-critical-section: reliably red under plain parallel. The cure is STRUCTURAL, not another per-test convention: every page-global writer (`set_measure`/`set_page_on`/`toggle`/`widen`/`narrow`, page.rs) acquires the ONE test lock internally under `cfg(test)`, reentrant per thread (`page::test_lock()`, a thread-local hold flag — a lock-holding test calling a writer is a no-op nested acquire), so a future test that writes the globals only transitively can never race a locked reader. Reader tests still take `page::test_lock()` across their read window; the raw `page::TEST_LOCK` mutex is gone — the fn is the only door. LOCK ORDER across suites (page always LAST): `theme::TEST_LOCK` → `fs::TEST_LOCK` → `page::test_lock()`. Plain parallel `cargo test` is reliable; no `--test-threads=1` retry needed. Law tests: `page::tests::test_lock_is_reentrant_and_the_outermost_guard_owns_the_release` / `writers_on_another_thread_block_while_the_lock_is_held`.
