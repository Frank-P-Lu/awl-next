...
- **Perf is measured, not guessed.** Bench hot-path work *before/after* with the hidden `--bench-perf` harness (`src/render/perfbench.rs`, median ns/call over `benches/fixtures/` for the traced paths — motion / ornaments / conceal / theme-reshape): record the BEFORE on the base, apply the fix, re-run for the AFTER delta, and keep the bench as a regression guard. Ship perf work *with the numbers*. For a GPU-memory question, build the same kind of headless soak loop sampling `MTLDevice.currentAllocatedSize` (via `device.as_hal::<wgpu::hal::api::Metal>()`) — a curve beats a guess.
- **Known flake:** `render::tests::theme_font_switch_reshapes_document` can fail under PARALLEL `cargo test` (a system-font cache race); it passes single-threaded / in isolation. Retry with `--test-threads=1` before treating it as a real failure.
