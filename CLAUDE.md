...
- **History timeline read-model:** `history::list()` prefers git history for a git-managed file, so PERIODIC awl snapshots taken inside a repo (`autosnapshot_secs`, `record_periodic`) do not surface in the timeline while git is available. The record contract is fixed; whether the timeline should merge both sources is an open question.
