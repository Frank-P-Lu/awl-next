...
- **Adding a `ViewState` field:** update EVERY `ViewState {` initializer — the FIVE production sites are `src/app/viewstate.rs` (the live App's `sync_view`), `src/bench.rs` (`view_for`), `src/capture/modes.rs`, `src/render/framebench.rs` (`live_view`), and `src/render/perfbench.rs` (`bench_view`); the `src/render/tests/mod.rs` `view()` helper is a sixth (test-only) — or the build breaks only at merge time (git auto-merges cleanly and then fails to compile).
