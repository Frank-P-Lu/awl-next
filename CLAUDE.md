...
- **Design-session flow (user rule 2026-07-22, full protocol in `.orchestrator/README.md`):** brainstormed decisions land as self-contained queue items (`DECIDED <date>` lines — a worker gets the decided thing, never the open question) committed with an `orchestrator: decisions` subject; git is the log — no decisions file. **`~/notes/` is the user's space: agents read there, never write.**
