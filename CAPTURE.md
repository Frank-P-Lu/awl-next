...
Schema `awl-capture/67` (was `/64`; timeline `/68`, held `/69`) adds the
TWO-TIER COMMENT tag to the syntax role vocabulary: `syn_spans` may now carry
...
| `syn_spans`    | SYNTAX HIGHLIGHTING: array of `[start_byte, end_byte, "tag"]` Alabaster role spans (`comment`/`comment_code`/`string`/`constant`/`definition` — `comment_code` is the two-tier commented-out-code tier, schema `/67`); empty for non-CODE buffers (`.env`/`.md`/`.txt`/unknown). Mutually exclusive with `md_spans` |
