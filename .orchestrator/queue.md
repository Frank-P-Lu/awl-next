...
1. **Poster-bars overlays preserve the live page — land it.** 🟡 IN PROGRESS — claude, 2026-07-22, branch codex/poster-bars-preserve-page (merge-train verify + land). Was 🟢 READY, claim released 2026-07-20 (was codex, blocked at the Terra cap). The implementation was RECOVERED after macOS wiped the `/private/tmp` worktree (the eight files sat uncommitted; the cap rejected the original `git add && git commit`): replayed from the Codex session log onto claim base `d6fa460`, now COMMITTED at `6b16685` on `codex/poster-bars-preserve-page`. Equivalence: exact diffstat match (8 files, 212+/184−, per-file identical) and byte-identical diffs against every in-session `git diff` capture. In-session evidence before the cap: focused + full suites green (2,490 passed, 11 ignored), capture/pixel/vision reported. Remaining: fresh worktree from the branch, re-run focused + full suites (the recovery was never re-compiled), sample captures across Mangrove/Firetail/Cassowary/Tawny, then the merge train. Original acceptance stands: source glyphs visibly survive; selected/unselected plates distinct and legible; PNG-pixel outcome laws; live feel human-confirmed.
2. **Frost DPI sibling fix — finish the test repair.** 🟡 IN PROGRESS — claude, 2026-07-22, branch codex/frost-dpi (test repair + merge-train land). Was 🟢 READY, claim released 2026-07-20 (was codex, blocked at the Terra cap; its `/tmp` worktree also died but the work is safe — branch `codex/frost-dpi` @ `12b79de`). Remaining scoped test-only repair: remove the temporary `eprintln!` in `src/render/tests/frost.rs`, measure 1×/2× pill EXTENTS (not absolute origins — those include the adaptive rail's fixed physical floor), rerun release + full suite, then merge. Acceptance: 1×/2× pixel geometry/density law with unchanged logical feel; live Retina taste human-confirmed.
