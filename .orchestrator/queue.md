...
- **Export save-dialog platform scope.** Codex asked whether Export's native save dialog should cover macOS AND Linux. Orchestrator recommendation 2026-07-20: yes, both — Linux is a first-class native target; one cross-platform seam, live-App-only and capture-gated (headless export takes an explicit path, no dialog). Awaiting the user's word.
- **Discussion-required interview (from `~/notes/awl-improvements.md`).** The user wants a ruthless interview then recommendations before any build: goto-menu semantics (default vs by-type identical; include headings? IntelliJ Shift-Shift model?); Finish-Buffer purpose/discoverability (it serves the `--wait`/EDITOR flow — hide it otherwise?); notes-vs-project launch flow (getting back to notes from a project is painful); bullet-list level readability strategy; table selection/copy (selection doesn't highlight or copy properly — show raw text when selecting?); link render (underline?); insert-date command (format? picker?); plates-under-text taste (pasted-7); CLAUDE.md simplification; overengineering-removal pass scope; meta-review ("are similar issues lurking; how do we catch these earlier"). Run as its own session; queue the winners.
