...
- **OS clipboard interop is COPY-only, updated by the WEB ESCAPE HATCHES
  round** (see the "What's stubbed / simplified" section above for the full
  writeup): copy/cut mirror out to `navigator.clipboard` best-effort;
  paste still reads the internal kill ring only (a logged, deliberate
  asymmetry — `readText`'s permission UX was judged not worth shipping).
