...
- **Rebindable navigation motions (settled 2026-07-10, WIDENED by the emacs-hands-on-Linux round below) — the "one `[keys]` line away" claim, now true.** TEN curated navigation motions are ordinary `commands.rs` catalog entries — palette-visible, rebind-menu-listed, `[keys]`-rebindable exactly like any other command: **Forward word** (`M-Right`), **Backward word** (`M-Left`), **Line start** (`Cmd-Left` · `C-a`), **Line end** (`Cmd-Right` · `C-e`), **Document start** (`Cmd-Up`), **Document end** (`Cmd-Down`) — the original six — plus **Forward char** (`C-f`), **Backward char** (`C-b`), **Next line** (`C-n`), **Previous line** (`C-p`), the last four bare-control motions, added so `[keys]`/`linux_keep_emacs` could reach them at all. The config template documents the retired Meta-letter word-motion chords as a straight `[keys]` restoration: `forward_word = ["M-Right", "M-f"]` / `backward_word = ["M-Left", "M-b"]`. Only the PLAIN arrow motions (Left/Right/Up/Down, no modifier) stay keymap-only, uncataloged — those dispatch via `resolve_named`'s static arms regardless of catalog membership, so there is no chord to name. Law tests: `commands::tests::catalog_motions_are_exactly_the_curated_navigation_set` (now the FULL `is_motion` set, no residual keymap-only motion), `motion_commands_are_all_present_named_and_rebindable`.
- **THE EMACS-HANDS-ON-LINUX ROUND (settled 2026-07-10) — a per-chord door, not a policy flip.** The Linux-native collision table (native-wins, see `keymap.rs`'s module doc) quietly displaced the bare-control emacs nav cluster (`C-f`/`C-b`/`C-n`/`C-p`/`C-a`/`C-e`) with Search-forward/Bold/New-note/Command-palette/Select-all/Inline-code — reported by the primary Linux user (Omarchy/Hyprland) as making the editor feel broken for an emacs hand, while `C-c`/`C-x`/`C-v` MUST stay native (Omarchy forwards Super+C as Ctrl+C for system clipboard). New config array `linux_keep_emacs` (e.g. `linux_keep_emacs = ["C-f", "C-b", "C-n", "C-p", "C-a", "C-e"]`) lists chords that keep their emacs meaning under `Convention::Linux` ONLY — each listed chord's native command yields JUST that chord (still reachable by palette/menu/its other chord); everything else keeps native-wins, unchanged. **Dispatch (`keymap.rs`):** `KeymapState::linux_keep` (a parsed `HashSet<(Key,ModifiersState)>`, built by `apply_linux_keep`/`with_overrides_and_keep`) gates the SAME `native` bool every native arm already reads (`self.native_down(state) && !self.linux_keeps(logical, state)`) — one gate, no new dispatch arms, so a kept chord falls straight through to the pre-existing static emacs arm. **Labels (`commands.rs`):** `join_slots_truthful` grew a Tier 4 alongside its existing web-reserved/Linux-displaced tiers — `linux_displaces_emacs_default` is now `keep`-aware (a kept chord is never "displaced"), and the NATIVE command that used to claim that chord stops advertising it (`linux_keeps_chord`, the ONE canonical-compare helper both directions share, in `keymap.rs`). `effective_bindings`/`visible_effective_bindings`/`BuildCtx` (+ `overlay::build`) all thread the keep-list through to the palette/rebind-menu. Mac ignores the key entirely (checked structurally — `linux_keeps` gates on `convention == Linux` even if the set happens to be non-empty). Live reload (`App::reload_config`) re-applies `apply_linux_keep` alongside `apply_overrides`; headless `--config` threads it through `keyspec::parse_keys_with` → `KeymapState::with_overrides_and_keep`, so a capture can assert it. Tests: `keymap::tests` (dispatch under a real Linux keymap, bad-entry leniency, reload-replaces-not-accumulates, Mac-inert), `commands::tests` (the two-sided label fix, per-chord not a policy flip, Mac-inert across the whole catalog), `config::tests` (parse/leniency/absent-is-empty).
...
- **THE KEYMAP FLAVOR ROUND (settled 2026-07-11) — the IDE-keymap-dropdown pattern, awl-narrow: two flavors only.** A new config `keymap = "native" | "emacs"` (string, absent/unrecognized = `"native"`, lenient like `caret_mode`/`dictionary`) is a WHOLE-CATALOG PRESET over the emacs-hands-on-Linux round's exact keep-list machinery — zero new dispatch logic, one new composition owner. `keymap = "emacs"` widens the effective `linux_keep_emacs` list to (every chord `keymap::LINUX_DISPLACED_LETTERS` names) ∪ (the user's own explicit `linux_keep_emacs` entries, deduped canonically) — `Config::effective_linux_keep()` is THE ONE OWNER of this composition, and `keymap::linux_emacs_preset_keep()` derives the preset FROM `LINUX_DISPLACED_LETTERS` itself (never hand-copied — a no-drift law test pins it). **Every call site that used to read `config.linux_keep_emacs` directly now reads `config.effective_linux_keep()` instead** (`App::new`'s keymap construction, `App::reload_config`/`refresh_rebind_overlay`, `keyspec::parse_keys_with`, both `BuildCtx` construction sites in `app/apply.rs`/`main/run.rs`) — `keymap.rs`/`commands.rs` themselves stay entirely unaware a "flavor" concept exists; they only ever see the already-composed `keep` list. `[keys]` stays the carve-out layer, unchanged and untouched by this round (a `[keys]` override is consulted before any static/keep arm — the exact seam the Omarchy recipe in the config template leans on: `keymap = "emacs"` + `[keys] copy/cut/paste = "C-c"/"C-x"/"C-v"` keeps the system clipboard chords native even under the emacs preset, since Omarchy/Hyprland forwards Super+C/X/V as Ctrl+C/X/V). **Labels are truthful for free:** `join_slots_truthful` (Tier 4, unchanged code) reads the SAME `effective_linux_keep()`-derived `keep` list dispatch uses, so the palette/rebind-menu/whichkey never lie about which chord actually fires. **Mac is inert** (law-tested over the whole catalog, mirroring `linux_keep_emacs_is_inert_on_mac_for_the_whole_catalog`) — no collisions exist there to keep. **Settings row:** a new "Keymap" `SettingKind::Toggle` row under the Keybindings category (corpus grew 25→26), NOT a plain bool config key (its value is `"native"`/`"emacs"`, not `"true"`/`"false"`) — `App::setting_toggle` special-cases `key == "keymap"` into `App::toggle_keymap_flavor`, which flips the in-memory `Config::keymap` mirror UNCONDITIONALLY (before attempting the disk write — deliberately NOT `self.reload_config()`, which re-READS from disk and would silently discard the flip on a path-less config), persists via the ordinary `persist_pref` owner, then re-applies the keymap LIVE from the updated config (`apply_overrides` + `apply_linux_keep` against the fresh `effective_linux_keep()`) — the same two calls `reload_config` itself makes. **Web: a documented session-only preference** (mirrors the "Edit config as text" web-hidden precedent) — a web build's `Config::empty()` has no path, so the disk write is silently a no-op there, but the flavor still takes effect immediately (the in-memory mirror + live keymap rebuild are unconditional) and lasts until the tab reloads. **Sidecar:** the `project` block (already `notes_root`/`workspace`-carrying) gains `keymap_flavor` (`"native"`/`"emacs"`, the EFFECTIVE value, verifiable via `--config` with no flags); schema bumped `163`→`164`. A default `--screenshot` (no `--root`, `project: null`) is unaffected. Tests: `keymap::tests` (the no-drift preset law, the whole-preset dispatch reversion for every displaced letter — not just the three named in spec — the non-displaced-chord no-op, the `[keys]` carve-out still winning), `commands::tests` (the two-sided whole-preset label fix, Mac-inert over the whole catalog, `effective_linux_keep` feeding `join_slots_truthful` identically to the bare preset), `config::tests` (parse absent/native/emacs/garbage, `effective_linux_keep`'s native/emacs/union/dedup shapes), `app.rs` (the Settings-row round-trip: flip, persist, live re-apply, round-trips back cleanly; the corpus includes the row).
...
### THE KEYMAP-DEFAULTS-AS-DATA ROUND — one embedded file, one splice, everything else unchanged

- **What:** user-decided ("everything unbound by default and specified by the data file"). Before this round a default chord existed in TWO hand-written places that a law test kept honest by re-verification: `commands.rs`'s `COMMANDS` catalog literal (`native`/`emacs` string fields) and `keymap.rs`'s hand-written dispatch arms (`resolve`/`resolve_named`/`resolve_char`/`resolve_c_x`/`resolve_c_c`) plus the standalone `LINUX_BUILTIN_KEEP` const. This round data-ifies the FIRST of those two — the catalog's chord VALUES — into one embedded TOML file; the dispatch MACHINERY (how a keypress becomes an `Action`) stays hand-written code, a deliberate, logged scope trim (see below).
- **`assets/keymap-defaults.toml`** (embedded via `include_str!`, `src/keymap_defaults.rs`, never read from disk, never user-facing) is now the ONE place a default chord VALUE lives: a `[commands]` table keyed by the same slug `commands::slug()` derives from a command's palette name, each entry a 2-element `[native, emacs]` array (native = mac-literal form, translated to Ctrl on Linux downstream exactly as before; emacs = OS-agnostic terse text) — plus a top-level `linux_builtin_keep = ["C-k"]` array, the former hand-written `keymap::LINUX_BUILTIN_KEEP` const, now data too. Every one of the catalog's 78 commands has an entry (even the all-empty palette-only ones) — "unbound by default" is literal: an entry absent from the file (impossible today, since every command IS listed) or with two empty strings has no default chord at all, full stop.
- **`keymap_defaults.rs`** parses the embedded file ONCE (`LazyLock`) into `command_defaults() -> &'static HashMap<String,(String,String)>` and `linux_builtin_keep() -> &'static [&'static str]`. A malformed embedded file PANICS at first access (the opposite leniency rule from `config.toml`'s user-facing parser, deliberately — this file ships inside the binary with no user in the loop to make a typo, so a bad shape is this codebase's own bug and must fail fast in CI/tests, not degrade silently).
- **`commands.rs`'s `COMMANDS`** is now `LazyLock<Vec<Command>>` (was `&'static [Command]`) — `Deref` keeps every existing `.iter()`/`COMMANDS[i]`/`.len()` call site unchanged; the only mechanical fallout was ~14 bare `for c in COMMANDS { .. }` loops (which cannot desugar against a `LazyLock`'s owned `Vec` the way they could against a `Copy` slice reference) becoming `for c in COMMANDS.iter() { .. }`. The array literal itself (renamed `COMMAND_SEED`, private) is otherwise BYTE-IDENTICAL in order/names/actions/comments/`native_only`/`web_only` — only its `native`/`emacs` string literals were blanked to `""` placeholders (never read; a law test, `command_seed_itself_carries_no_residual_chord_literals`, catches a reintroduced literal even though it would be silently discarded by the splice). `COMMANDS`'s construction closure looks up each seed's slug in `keymap_defaults::command_defaults()` and splices the result in via `Box::leak` (a one-time, ~80-entry startup cost, memoized by `LazyLock` — never repeated).
- **`keymap::LINUX_BUILTIN_KEEP`** (the const) became `keymap::linux_builtin_keep()` (a fn delegating to `keymap_defaults::linux_builtin_keep()`) — same value, same `&'static [&'static str]` shape, every call site needed only `()` added.
- **STAYS CODE (policy, not assignment — logged scope trim, not attempted this round):** the plain-arrow motions (uncataloged, dispatched by `resolve_named`'s static arms regardless of any catalog/data entry), the Cmd-to-Ctrl convention TRANSLATION rule + its override table (`commands::LINUX_NATIVE_OVERRIDE`), the native-wins Linux collision table (`keymap::LINUX_DISPLACED_LETTERS`), the prefix-sequence dispatch machinery itself (`resolve`/`resolve_named`/`resolve_char`/`resolve_c_x`/`resolve_c_c` — the actual keypress-to-`Action` matching, which reads winit `Key`/`ModifiersState` and is inherently procedural, not just data), `webreserved`, and `commands::WEB_ALTERNATE`. **Honest gap, not silently declared "done":** because the dispatch arms are still hand-written and were NOT rewritten to consult the embedded file, `commands::tests::catalog_and_keymap_agree_on_every_default_chord` is NOT "trivially true by construction" in the way the round's ideal end-state would make it — it remains a real, valuable RE-VERIFICATION that keymap.rs's literal `'s'`/`'z'`/… match arms still agree with the now-data-sourced catalog values, kept unmodified and still load-bearing. A follow-up round could go further (seed `KeymapState`'s override maps directly from the embedded file and delete the corresponding static arms), but that is a genuine rewrite of the input-dispatch core with real regression risk, out of proportion to this round's effort budget — flagged, not attempted.
- **Law tests on the data (`commands.rs`):** the file parses (`keymap_defaults::tests`); every TOML slug names a real catalog command AND every catalog command appears in the TOML, in both directions, plus an entry-COUNT match (`every_defaults_toml_slug_names_a_real_catalog_command`, `every_catalog_command_appears_in_the_defaults_toml_or_is_unbound`, `defaults_toml_has_no_stale_slugs_and_no_duplicates`); the splice is exact (`commands_splices_the_embedded_defaults_verbatim`); `COMMAND_SEED` carries no residual literal chord (`command_seed_itself_carries_no_residual_chord_literals`); the max-2-chords cap is structural (the parser panics past 2 elements); the pairwise default-chord conflict sweep (`no_two_catalog_commands_share_a_default_chord`, pre-existing, unmodified) now runs over the DATA-SOURCED `COMMANDS` for free.
- **Behavior-identical, proven directly:** a full `COMMANDS` dump (name/action/native/emacs/native_only/web_only, stable order) taken from this branch is byte-identical to the same dump taken from the pre-refactor base commit (verified via a temporary `#[ignore]`d snapshot test run on both trees, `--ignored --nocapture`, diffed) — since every downstream consumer (`effective_bindings`, `join_slots_truthful`, the menu roster, the GUIDE reference table, the rebind menu, whichkey) is untouched code that is a pure function of this exact data, an identical dump is sufficient to prove the whole label/dispatch-agreement surface is behavior-identical without re-deriving every derived view. The snapshot test itself was NOT kept as a permanent law test (it would just re-assert what `catalog_and_keymap_agree_on_every_default_chord` + the new data-law tests already cover going forward, with no NEW failure mode of its own to guard).
- **Gates:** `cargo build` / `cargo test` (native + `AWL_CONVENTION_FORCE=linux`) / `cargo build --target wasm32-unknown-unknown` all green at 0 warnings; `scripts/web-smoke.sh` L1+L2 OK; `GUIDE.md`'s generated keys-reference table is byte-unchanged (`guide::tests::generated_keys_reference_matches_catalog` still passes without a regen, since the resolved chord VALUES the table reads are unchanged).
...
## Adaptive-column placement (`render/geometry.rs` + `render/rowlayout.rs`) — the outline gets a real rail under width pressure

- **What:** on a small screen, page mode's SYMMETRIC centered column used to cramp the persistent margin outline into a too-thin left margin while the right margin — which holds no chrome at all — sat equally thin and simply unused. `TextPipeline::column_left` (`render/geometry.rs`) is now the ONE owner of an ADAPTIVE placement policy: automatic, no config knob (user-decided) — the column shifts RIGHT under pressure to grant the outline a real rail, taking the space from the otherwise-empty right margin, and shifts back to plain symmetric centering the instant there's room to spare. Every downstream reader of the column (caret/selection/washes, hit-test, the page-edge drag handle, the corner readouts, the bottom-left gutter) composes this for free — they all already read through `column_left()`, so there is no parallel geometry to keep in sync.
- **The pure policy — `render::geometry::adaptive_column_left`** (pure fn, no GPU): inputs are the canvas width, the ZOOM-INDEPENDENT glyph advance, page-on, the measure, whether the outline WANTS the rail (`TextPipeline::outline_wants_rail` — feature on, page mode on, a markdown buffer with ≥1 heading, the SAME gate `outline_layout`'s own horizontal hide check reads, so the two can never disagree), the outline's preferred rail width in px, the margin gap, and the text-left pad. Three regimes, ONE formula (`desired_left.min(max_left).max(symmetric_left)`), no explicit branch for the third:
  - **WIDE** (the symmetric column already seats the outline's preferred rail): an EXACT passthrough to the pre-existing `column_left_for` — byte-identical to the pre-round column, the hard law this round is built around (verified via a real before/after capture diff at `--measure 40` on the 1200px canvas: PNG + sidecar both byte-identical).
  - **NARROW** (symmetric can't seat the preference, but there's margin to spare): the column shifts right — up to the outline's full preferred rail, capped so `RIGHT_MARGIN_BREATH` (= `PAGE_MIN_PAD`) always survives on the right. The column's WIDTH (its measure) is NEVER touched — only where it sits moves; the writing column keeps its exact character count either way.
  - **NARROWEST** (even the breathing cap can't be honored): the `min`/`max` chain settles back on the plain symmetric left with no separate branch — the column "re-centers", which naturally starves `outline_layout`'s own `avail_chars < rowlayout::OUTLINE_MIN_CHARS` floor check, so the outline auto-hides exactly as it already did — the hide threshold and the shift threshold read off the SAME `left`, so they can never drift apart.
- **`rowlayout::OUTLINE_PREFERRED_CHARS`** (= `OUTLINE_MIN_CHARS * 3`, a TASTE TUNABLE flagged for live review) — the outline's comfortable target rail width, deliberately expressed as a multiple of the existing hard floor rather than a parallel magic number, so the two constants can only ever be retuned together.
- **Transition:** INSTANT this round (no glide) — a toggle/resize re-runs the same pure placement and the column jumps to its new `left` on the very next frame, exactly like any other geometry recompute (measure change, zoom, DPI). A glide is a banked taste call, not attempted (Reduce Motion interplay would need a decision on whether a glide is exempt like the existing spring-based caret juice, or plain — flagged, not built).
- **Sidecar:** NO schema bump — `page.column.left`/`page.column.width` (`TextPipeline::page_geometry`) already existed and now simply report the shifted value, since they delegate to `column_left()`/`column_width()` verbatim.
- **Tests:** `render::geometry::tests::adaptive_*` (pure, exhaustive over WIDE / NARROW-grants-full-rail / NARROW-caps-at-breathing-floor / NARROWEST-recenters / outline-not-wanted / page-off / the exact WIDE↔NARROW threshold boundary / the column-never-shrinks invariant). `render::tests::outline::outline_shifts_the_column_right_under_pressure_and_gets_its_rail` is the end-to-end regression test for the ROUND'S OWN motivating bug (a markdown doc with headings at the standard 1200px canvas + the DEFAULT 70-char prose measure — previously HIDDEN outright, `left=96` giving 5 avail chars < the 6-char floor; now shifts to `left=176`, granting 12 avail chars, and the outline draws). `outline_hides_below_the_narrow_margin_floor` was updated (superseding its old symmetric-only fixture, which the round's own fix now makes pass — a genuinely NARROWEST window/measure combo, where the measure itself doesn't fit the window at all, replaces it so the graceful-hide law still has a real regression guard).
- **LIVE-ONLY (needs human confirmation):** the actual FEEL of an outline toggle / window resize on a real small laptop screen — the harness proves the placement formula and the byte-identical wide law via captures + unit tests, not the in-motion transition on a real display.
...
### THE LINKS V2 ROUND — Cmd-K insert/edit link, C-k default rework (`actions/link.rs` + `keymap.rs` + `commands.rs`)

- **What:** spends the Cmd-K chord the keybinding-idiom audit reserved for exactly this (the writer-app convention — Bear/Craft/Notion/Things/Ulysses/Slack all bind insert-link there). A new **Insert link…** command (`Action::InsertLink`, markdown buffers only, palette-visible, independently rebindable via `[keys]`) summons a URL minibuffer and applies the edit as ONE atomic undoable transform (`actions/link.rs`, mirroring `actions/format.rs`'s pure-transform-in/`FormatResult`-out shape — same `Buffer::apply_format` primitive a format toggle uses).
- **Three modes, decided purely from buffer state (`link::plan`), no mutation until Enter:** (1) an ACTIVE SELECTION wraps that exact span — `[selection](url)`; (2) no selection, but the caret sits inside an existing link (`markdown::link_at_full`) — EDIT mode, re-prompts with that link's own current URL (never the kill-ring head — editing an existing link shows what's there, not an unrelated prefill), rewriting the same range on commit; (3) neither — inserts empty `[](url)` markup at the caret, caret landing between the brackets ready to type link text. A URL sitting in the kill ring (`Buffer::kill_buffer`) prefills the minibuffer IFF it looks like a URL (`buffer::is_url`) — a taste call, flagged: a stale kill-ring URL might not be what the user means to link to.
- **Paste-URL-over-selection already existed before this round** (the same `buffer::is_url` helper) — pasting a bare URL over an active selection wraps the selection as link text automatically; Links v2 reuses that helper rather than duplicating URL-shape detection.
- **Cmd-K stays Insert link on Mac, unconditionally** — `Convention::Mac` never consults the Linux keep-list at all. On **Linux, Cmd-K would have displaced Ctrl-K's kill-line meaning exactly like every other native-doors chord** (`keymap::LINUX_DISPLACED_LETTERS`), but the user rejected that trade outright: kill-line is too load-bearing for emacs hands to lose by default. So `k` is deliberately NOT in `LINUX_DISPLACED_LETTERS` — instead `keymap::LINUX_BUILTIN_KEEP` (currently just `["C-k"]`) is a THIRD, unconditional-keep case, seeded on every `KeymapState::apply_linux_keep` call regardless of `linux_keep_emacs`/the `keymap` flavor preset. Practical upshot: **Ctrl-K stays kill-line on Linux in BOTH keymap flavors, with no config needed** — Insert link simply has no effective Linux binding by default. **Reclaim recipe:** `[keys] insert_link = "C-k"` is still one line away for a Linux hand who explicitly wants the trade (a `[keys]` override is consulted before the built-in-keep floor, same as every other override). `Config::effective_linux_keep()` seeds the same unconditional `C-k` keep into the label-truth half too, so the palette/rebind-menu label agrees with real dispatch.
- **Law tests:** `keymap::tests` (the built-in-keep dispatch table, the reclaim-via-`[keys]` override winning over the floor, Mac stays Cmd-K unconditionally), `commands::tests` (Insert link catalog entry, Linux label-truth agreeing with `LINUX_BUILTIN_KEEP`), `actions::tests` (`link::plan`'s three-mode decision, `link::commit`'s text-building math, the URL-looks-like-a-URL kill-ring prefill gate).
...
## Docs voice (user-set, 2026-07-11)
User-facing docs (CREDITS, GUIDE, welcome/tour seeds, site pages) are **matter-of-fact**: tables and short declarative sentences, no gratitude prose, no editorializing adjectives, no stylistic flourishes ("weightless", "lovingly") — nothing that reads as AI-written filler. Facts trace to verified sources. Warmth is carried by the product, not the paperwork. PHILOSOPHY/DESIGN keep their personal register (the author's own voice, hand-edited).
...
- **2026-07-12 audit round (via `scripts/audit.sh`, overdue since 2026-07-06):** NO new findings, and nothing to fix — the SAME three no-non-major-path advisories, all re-confirmed unchanged and carried forward as-is, no version bumped this round: **RUSTSEC-2026-0194 + RUSTSEC-2026-0195** (`quick-xml` still pinned at 0.39.4, patched ≥0.41.0) — re-verified the "no path" claim directly rather than just re-reading last round's note: `cargo update -p quick-xml --dry-run`, `-p wayland-scanner --dry-run`, and `-p smithay-client-toolkit --dry-run` each lock 0 packages, confirming the `winit = "0.30"` ceiling still caps `quick-xml` below 0.41 via that chain; still low practical risk, the parsed XML is the build-time-bundled Wayland protocol spec, never attacker input. **RUSTSEC-2026-0192** (`ttf-parser` still 0.25.1, unmaintained, no patched version) — `cargo update -p ttf-parser --dry-run` and `-p fontdb --dry-run` likewise lock 0 packages; still no path short of a font-parser swap. `anyhow`/`memmap2` hold at their 2026-07-05 patched versions (1.0.103 / 0.9.11, confirmed in `Cargo.lock`). `cargo audit` scanned 340 crate dependencies (up from 326 on 2026-07-06 — organic growth from the many feature merges landed since: the daemon, the macOS menu bar, session restore, clipboard support, and the CJK font rounds all pulled in new crates; not itself a supply-chain concern, not investigated further) against 1160 advisories (up from 1156); exit 1 (2 vulns + 1 allowed warning) is the expected steady state, unchanged from the last two rounds, until an upstream `winit`/`cosmic-text` release picks the fixes up transitively. No dependency version changed, so `THIRD-PARTY-LICENSES.md` was left untouched (nothing to regenerate).
...
## Branches & worktrees
- **The development branch is LOCAL `main`.** Origin's default branch flipped to `main` on 2026-07-10 — `git remote show origin` reports `HEAD branch: main` straight from GitHub. The old trap (origin's default pointing at `master`, a leftover from the repo's creation) is resolved upstream; `master` may still exist as a stale remote branch, kept around pending the user's own deletion call — never base new work on it. Note the local clone's CACHED `origin/HEAD` symref can lag behind the real upstream default until refreshed (`git remote set-head origin -a`), so don't trust `refs/remotes/origin/HEAD` blindly — `git remote show origin`'s live query is the source of truth. Base new work on local `main`, which may run AHEAD of `origin/main` between trains. **PUSH POLICY (user-updated 2026-07-11):** once a merge train lands GREEN on local `main` (full suite, both conventions, wasm — the normal gates), pushing `main` to origin is AUTHORIZED and expected — batch pushes per train rather than per-commit (each push burns a full CI run; mac runners bill 10× on the private repo). TAGS and RELEASES still require the user's explicit word every time — a tag publishes distributables; never tag/release on standing authority. Worktree branches are never pushed.
...
- **Test-coverage backlog:** the audited, risk-ranked list of ~35 further missing tests lives in the orchestration board (`.Codex/orchestrator/queue.md`) — the top-10 round landed; the rest trickle.
